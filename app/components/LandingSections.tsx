...
export function MagneticCursor() {
  // Cursor magic disabled – keep native cursor for a cleaner feel.
  return null;
}
...
export function RadioSection() {
  ...
  const videoY = useTransform(scrollYProgress, [0, 1], [-18, 18]);
  const videoScale = useTransform(scrollYProgress, [0, 1], [1.02, 0.98]);
      ...
      <div className="mx-auto max-w-6xl px-6 py-24 sm:px-10 sm:py-28">
        ...
        >
          ...
          <p className="max-w-md text-sm leading-relaxed text-black/60 sm:text-base">
            Keynote‑style storytelling with a floating playback bar and captions
          ...
          <div className="relative overflow-hidden rounded-[2.1rem]">
            <motion.div style={{ y: videoY, scale: videoScale }}>
              <video
                ...
                loop
                preload="metadata"
                poster="https://images.unsplash.com/photo-1527443154391-507e9dc6c5cc?auto=format&fit=crop&w=2200&q=80"
              ...
              <div className="glass-ios-dark glass-edge noise flex items-center justify-between gap-4 rounded-[1.6rem] px-5 py-4">
                <div className="flex items-center gap-4">
                  <motion.button
                    ...
                    className="flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-white shadow-[0_12px_30px_rgba(0,0,0,0.55)]"
                ...
                <div className="hidden items-center gap-3 sm:flex">
                  ...
                  <button
                    className="group relative h-2 w-56 overflow-hidden rounded-full bg-white/15"
                    onClick={(e) => {
                  ...
                  >
                    <motion.div
                      ...
                      transition={{ type: "spring", stiffness: 120, damping: 26 }}
                    />
                    <div className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-200 group-hover:opacity-100 [background:radial-gradient(circle_at_20%_50%,rgba(255,255,255,0.35),transparent_55%)]" />
                  </button>
                  <span className="text-xs tabular-nums text-white/60">
                    {durationText}
                  </span>
...
export function LibrarySection() {
  return (
    <section id="library" className="bg-transparent">
      <div className="mx-auto max-w-6xl px-6 pb-24 pt-12 sm:px-10 sm:pb-28">
        <motion.div
          initial={{ opacity: 0, y: 40, scale: 0.98 }}
          whileInView={{ opacity: 1, y: 0, scale: 1 }}
          viewport={{ once: true, amount: 0.35 }}
        ...
        >
          <div>
            ...
            <h2 className="text-3xl font-semibold tracking-tight sm:text-4xl">
              A family of quiet objects.
            </h2>
          </div>
          <p className="max-w-md text-sm leading-relaxed text-black/60 sm:text-base">
            Three expressions of Luna, tuned for studio days, city evenings, and
    ...
    <section className="bg-transparent">
      <div className="mx-auto max-w-6xl px-6 py-24 sm:px-10 sm:py-28">
        ...
        >
          ...
          <p className="max-w-md text-sm leading-relaxed text-black/60 sm:text-base">
            A curated set of minimal add‑ons—built like jewelry for your setup.
          </p>
        </motion.div>
        ...
        <div className="glass-ios glass-edge noise mx-auto flex max-w-3xl items-center justify-between gap-4 rounded-[2rem] px-6 py-6">
          <div>
            ...
            <p className="mt-2 text-sm leading-relaxed text-black/60">
              A product finder panel. Next: wire it to real data and filters.
            </p>
          </div>
...
function GlowButton({ label, primary }: { label: string; primary?: boolean }) {
  return (
    <motion.button
...
function NewCard({ title, copy, image }: { title: string; copy: string; image: string }) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="relative">
        ...
        <div className="mt-6 space-y-3">
          {highlights.map((h) => (
            <div key={h} className="flex items-center gap-3 text-sm text-white/70">
...
function AccessoryCard({ name, price, image }: { name: string; price: string; image: string }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 22, scale: 0.98 }}
      whileInView={{ opacity: 1, y: 0, scale: 1 }}
...
function FeatureBentoCard({
  ...
  const springX = useSpring(rotateX, { stiffness: 80, damping: 20, mass: 0.6 });
  const springY = useSpring(rotateY, { stiffness: 80, damping: 20, mass: 0.6 });
...
function ProductCard({ label, title, copy, chip, tag, image }: ProductCardProps) {
  const rotateX = useMotionValue(0);
  const rotateY = useMotionValue(0);
